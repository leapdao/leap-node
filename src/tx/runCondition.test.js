...
test('verify spending condition', async () => {
  ...
  condition.inputs[0].setMsgData(
    ...
    `${sig.r.toString('hex')}${sig.s.toString('hex')}${vBuf.toString('hex')}` + // signature
  ...
  const inputMap = {};
  [inputMap[condition.inputs[0].prevout.getUtxoId()]] = deposit.outputs;

  await runCondition(colorMap, condition, inputMap);
});
