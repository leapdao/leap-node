...
module.exports = async (colorMap, tx, inputMap) => {
  ...
  // check that hashes of scripts matche spent outputs
  for (let i = 0; i < tx.inputs.length; i += 1) {
    const scriptHash = utils.ripemd160(tx.inputs[i].script);
    const outputAddr = inputMap[tx.inputs[i].prevout.getUtxoId()].address;
    if (`0x${scriptHash.toString('hex')}`.localeCompare(outputAddr)) {
      return Promise.reject(
        new Error(
          `hash does not match: ${scriptHash.toString('hex')} ${outputAddr}`
        )
      );
    }
  }
